...
class ParticleFilter(InferenceModule):
  ...
  def initializeUniformly(self, gameState):
    ...
    for x in range(0, self.numParticles):
      self.listParticles.append(random.choice(self.legalPositions));
  ...
  def observe(self, observation, gameState):
    ...
    allPossible = util.Counter()
    prevBeliefs = self.getBeliefDistribution()


    if noisyDistance == None:
      print "None"
      self.listParticles = self.numParticles * [self.getJailPosition()]

    else:
      for p in self.legalPositions:
        trueDistance = util.manhattanDistance(p, pacmanPosition)
        if emissionModel[trueDistance] > 0: 
          allPossible[p] += prevBeliefs[p] * emissionModel[trueDistance]
      ...
      if list(allPossible) == [0]:
        ...
        tempList = []
        for x in range(0, self.numParticles):
          tempList.append(util.sample(self.beliefs));
        self.listParticles = tempList
  ...
  def elapseTime(self, gameState):
    ...
    newList = []
    for x in range(0, self.numParticles):
      oldPos = self.listParticles[x]
      newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
      newList.append(util.sample(newPosDist))

    self.listParticles = newList
...
class JointParticleFilter:
  ...
  def initializeParticles(self):
    ...
    # util.raiseNotDefined()
    # self.listParticles = []
    tempParticles = self.numParticles*[0]
    for x in range(self.numParticles):
      tempTuple = ()
      for y in range(self.numGhosts):
        tempTuple += (random.choice(self.legalPositions),)
      tempParticles[x] = tempTuple
    self.particles = tempParticles   
    # print "list: ", self.particles
    ...
    for oldParticle in self.particles:
      ...
      for i in range(self.numGhosts):
        newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle),
                                                   i, self.ghostAgents[i])
        newTuple += (util.sample(newPosDist),)

      newParticles.append(tuple(newTuple))

...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    for x in range(0, len(livingGhostPositionDistributions)):
      maxPosition = (0,0)
      maxDistribution = 0.0
      for pos, dist in livingGhostPositionDistributions[x].iteritems():
        if dist > maxDistribution:
          maxDistribution = dist
          maxPosition = pos

      newDistance = self.distancer.getDistance(maxPosition, pacmanPosition)
    ...
    for a in legal:
      ...
      newDist = self.distancer.getDistance(successorPosition, minPosition)
      if newDist < optDistance:
        optDistance = newDist
        optAction = a
    return optAction
